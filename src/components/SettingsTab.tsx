    ...
    <div className={`space-y-4.5 animate-fade-in px-1 select-none pb-12 font-sans ${darkMode ? 'text-white' : 'text-slate-800'}`}>
      ...
      <div className={`p-4 border rounded-2xl shadow-sm transition-all ${
        ...
        <p className={`text-[10px] leading-relaxed select-none ${darkMode ? 'text-slate-450' : 'text-slate-500'}`}>
          🎵 Plays a relaxing, hardware-synthesized, low-volume generative chord progression loop utilizing pure sine oscillators to maximize focus and spatial calm during your typing workouts. Includes automatic crossfades.
        </p>
      </div>
        ...
        <div className="space-y-3.5">
          <div className="flex justify-between items-center p-2.5 rounded-xl bg-slate-50/5 border border-slate-200/5 hover:border-slate-200/10 transition-all">
            <span className="text-[11px] font-bold tracking-wide uppercase font-mono">
              {darkMode ? "Dark Mode Activated" : "Bright Mode Activated"}
            </span>
      ...
      {/* 6. DAILY PRACTICE DYNAMIC ALERTS (ALIGNED WITH PWA FOR REAL CONVENIENCE) */}
      <div className={`p-4 border rounded-2xl shadow-sm transition-all ${
        darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-150'
      }`}>
        <div className="flex justify-between items-center gap-2 mb-3.5">
          ...
          <button
            ...
            className={`cursor-pointer px-3 py-1.5 rounded-xl text-[9px] font-bold border transition-colors uppercase ${
              remindersEnabled 
                ? `${theme.fill} ${theme.border} font-bold` 
                : darkMode 
                  ? 'bg-slate-950 border-slate-800 text-slate-505' 
                  : 'bg-slate-50 border-slate-250 text-slate-400'
            }`}
          >
        ...
        {remindersEnabled && (
          <div className="space-y-3.5 animate-scale-up">
            {/* Clock inputs */}
            <div className="flex justify-between items-center p-2.5 rounded-xl bg-slate-52/5 border border-slate-210/5">
              ...
              <input
                ...
                className={`rounded-lg px-2 py-1 test-time-input font-bold text-xs select-none cursor-pointer focus:outline-none border ${
                  darkMode
                    ...
                    : 'bg-slate-50 border-slate-200 text-slate-750 focus:border-indigo-500'
                }`}
              />
            </div>
            ...
            {notifState === 'success' && (
              <div className="p-2.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-550 rounded-xl text-[10px] leading-relaxed text-center font-bold animate-pulse">
      ...
      {/* SYSTEM INFO SECTION */}
      <div className={`p-4 border rounded-2xl leading-normal space-y-1 shadow-sm transition-all ${
        ...
        <p className={`text-[10px] leading-relaxed select-none ${darkMode ? 'text-slate-500' : 'text-slate-550'}`}>
          {dict.specs_desc}
        </p>
      </div>
